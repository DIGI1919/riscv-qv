...
int32_t message_print(int top_mtohost){
  static int buffer ;
  int32_t return_n=0;
  if((top_mtohost!=0)&(buffer!=top_mtohost)){
    return_n = top_mtohost;
  }
  else{return_n = 0;}
  buffer = top_mtohost;
...
int main(int argc, char** argv) {
  ...
  //message save
  vector<int32_t> rv_output_code;
  vector<int32_t> rv_output_cyc;
  ...
  do {
    ...
    buffer = message_print(top->io_host_tohost);
    if (buffer!=0)
    {
      rv_output_code.push_back(buffer);
      rv_output_cyc.push_back(main_time/2);
    }
    
   
  } while(!(top->io_host_tohost==(int32_t)-1) && main_time < timeout);
  ...
  std::string messages_type;
  for (size_t i = 0; i < rv_output_code.size(); i++)
  {
    if (i ==(rv_output_code.size()-1) )
    {
      messages_type = "OVER = ";
    }else if(i ==(rv_output_code.size()-2) ){
      messages_type = "INST = ";
    }else{
      messages_type = "CODE = ";
    }
    
    cerr << "RETURN = " << i <<" "<< " cycle =" << rv_output_cyc[i] << " "<< 
           messages_type << hex <<rv_output_code[i] << endl;
  }
